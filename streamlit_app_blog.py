...
import time
import shutil
from src.llm import LLMfromPDF
...
if "pdf_qa_model" not in st.session_state:
    st.session_state["pdf_qa_model"]:LLMfromPDF = LLMfromPDF() ## Intialisation
...
def load_llm(llm,load_in_8bit):
    ...
    if llm == LLM_OPENAI_GPT35:
        ...
        return LLMfromPDF.create_flan_t5_small(load_in_8bit)
    elif llm == LLM_FLAN_T5_BASE:
        return LLMfromPDF.create_flan_t5_base(load_in_8bit)
    elif llm == LLM_FLAN_T5_LARGE:
        return LLMfromPDF.create_flan_t5_large(load_in_8bit)
    elif llm == LLM_FASTCHAT_T5_XL:
        return LLMfromPDF.create_fastchat_t5_xl(load_in_8bit)
    elif llm == LLM_FALCON_SMALL:
        return LLMfromPDF.create_falcon_instruct_small(load_in_8bit)
    else:
...
def load_emb(emb):
    if emb == EMB_INSTRUCTOR_XL:
        return LLMfromPDF.create_instructor_xl()
    elif emb == EMB_SBERT_MPNET_BASE:
        return LLMfromPDF.create_sbert_mpnet()
